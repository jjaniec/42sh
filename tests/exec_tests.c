...
void	exec_tests(char ***envp_ptr)
{
	...
	compare_sh_21sh_outputs("Simple AND", "/bin/echo a && /bin/echo b", NULL);
	compare_sh_21sh_outputs("AND w/ cd;", "pwd; cd .. && /bin/echo a; /bin/echo b", NULL);
	compare_sh_21sh_outputs("AND w/ OR", "/bin/echo a && pwd || /bin/echo ABC", NULL);
	compare_sh_21sh_outputs("OR w/ AND", "/bin/echo a || pwd && /bin/echo ABC", NULL);
	compare_sh_21sh_outputs("AND x2", "/bin/echo a && pwd && /bin/echo ABC", NULL);
	...
	compare_sh_21sh_outputs("Pipes 3 - redirs", "/bin/echo a 2>&1 | cat", NULL);
	compare_sh_21sh_outputs("Pipes 4 - redirs", "/bin/echo a 2>/dev/null 3>/dev/null | cat", NULL);
	...
	compare_sh_21sh_outputs("TK_LESS 1 - Simple", "cat < "TESTS_TMP_FILENAME, NULL);
	compare_sh_21sh_outputs("TK_LESS 2 - Pipe", "cat < "TESTS_TMP_FILENAME" | cat", NULL);
	compare_sh_21sh_outputs("TK_LESS 3 - Multiple pipes", "cat < "TESTS_TMP_FILENAME" | cat | cat | cat|cat|cat|cat", NULL);
	compare_sh_21sh_outputs("TK_LESS 4 - OR", "ls /zzz || cat < "TESTS_TMP_FILENAME" |cat ", NULL);
	compare_sh_21sh_outputs("TK_LESS 5 - AND", "cat a && cat < "TESTS_TMP_FILENAME" |cat ", NULL);
	compare_sh_21sh_outputs("TK_LESS 6 - OR - AND - Pipes", "cat a && cat < "TESTS_TMP_FILENAME" |cat && cat "TESTS_TMP_FILENAME" | cat || cat 'this should not append'; cat <"TESTS_TMP_FILENAME" | cat", NULL);
	compare_sh_21sh_outputs("TK_LESS 7 - rewrite input redir to same file", "cat <"TESTS_TMP_FILENAME" 2>&1 | cat >"TESTS_TMP_FILENAME, NULL);
	...
	compare_sh_21sh_outputs("TK_TLESS Here-documents 1 - Simple", "/bin/cat <<< lol", NULL);
	compare_sh_21sh_outputs("TK_TLESS Here-documents 2 - Simple", "/bin/cat <<< diajdiwajdoiwdjdwadjaiwdjwaidj", NULL);
	compare_sh_21sh_outputs("TK_TLESS Here-documents 3 - Simple w/ lexer quotes concats", "/bin/cat <<< aaaa\"bbbbb\"'ccccc'ddddd", NULL);
	compare_sh_21sh_outputs("TK_TLESS Here-documents 4 - OR", "/bin/cat <<< lol || /bin/cat <<< lal", NULL);
	compare_sh_21sh_outputs("TK_TLESS Here-documents 5 - AND", "/bin/cat <<< lol && /bin/cat <<< lal", NULL);
	compare_sh_21sh_outputs("TK_TLESS Here-documents 6 - OR & AND", "/bin/cat <<< lol || /bin/cat <<< lal && /bin/cat <<< lel", NULL);
	compare_sh_21sh_outputs("TK_TLESS Here-documents 7 - Pipes", "/bin/cat <<< lol | cat", NULL);
	compare_sh_21sh_outputs("TK_TLESS Here-documents 8 - Pipes", "/bin/cat <<< lol | cat | cat | cat | cat", NULL);
	compare_sh_21sh_outputs("TK_TLESS Here-documents 9 - Pipes", "/bin/cat <<< lol | cat", NULL);
	compare_sh_21sh_outputs("TK_TLESS Here-documents 10 - Pipes w/ AND & OR", "/bin/cat <<< lol | cat | cat <<< lal | cat <<< lql && /bin/echo lsl | cat <<< lel || /bin/echo lul | cat <<< lzl", NULL);

	compare_sh_21sh_outputs("Builtins 1 - echo", "echo ls", NULL);
	compare_sh_21sh_outputs("Builtins 2 - echo multi args", "echo ls ls ls ls ls ls", NULL);
	compare_sh_21sh_outputs("Builtins 3 - echo no args", "echo", NULL);
	compare_sh_21sh_outputs("Builtins 4 - Pipes (w/ echo)", "echo ls | cat", NULL);
	compare_sh_21sh_outputs("Builtins 5 - Redirs (w/ echo)", "echo ls > /tmp/a && cat < /tmp/a", NULL);
	compare_sh_21sh_outputs("Builtins 6 - Pipes & redirs (w/ echo)", "echo ls / /abc 2> /tmp/a; cat < /tmp/a", NULL);
	compare_sh_21sh_outputs("Builtins 7 - cd", "cd / && pwd", NULL);
	compare_sh_21sh_outputs("Builtins 8 - cd", "cd / && cd dev && pwd", NULL);
	compare_sh_21sh_outputs("Builtins 9 - cd", "cd ~ && cd /dev && pwd", NULL);
	compare_sh_21sh_outputs("Builtins 10 - cd", "cd ~ && cd /dev && cd .. && pwd", NULL);

	compare_sh_21sh_outputs("Builtins 11 - env", "env | grep -v _", NULL);
	//compare_sh_21sh_outputs("Builtins 12 - env w/ T_ENV_ASSIGN", "TMP=test env | grep TMP", NULL); // 15/09: Not implemented
	compare_sh_21sh_outputs("Builtins 13 - env 2", "env | grep -v _ | cat", NULL);
