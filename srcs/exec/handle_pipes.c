...
/*   Updated: 2018/10/21 00:25:00 by jjaniec          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		redir_output_to_pipe(t_ast *last_pipe, bool take_parent)
{
	log_debug("PID %zu: Pipe on left", getpid());
	if (take_parent)
	{
		handle_redir_fd(STDOUT_FILENO, *(&(last_pipe->parent->data[1][sizeof(int)])));
		log_close(last_pipe->parent->data[1][0]);
	}
	else
	{
...
int			handle_pipes(t_ast *node)
{
	...
	if ((last_pipe_node = get_last_pipe_node(node)))
	{
		...
		{
			redir_output_to_pipe(last_pipe_node, false);
